#!/usr/bin/env bash
# Builds a signed, aligned APK using only Maven-Central + Debian sourced tools.
# No Android SDK manager / Google Maven required.
set -euo pipefail
...
PKG_PATH="io/berilo/hashishhistory"
OUT="$BUILD/hashish-history.apk"
...
if [ ! -f "$KS" ]; then
  keytool -genkeypair -keystore "$KS" -storepass android -keypass android \
          ...
          -dname "CN=Hashish History, O=berilo.io, C=US" >/dev/null 2>&1
fi
